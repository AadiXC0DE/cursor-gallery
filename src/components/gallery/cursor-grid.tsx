...
export function CursorGrid({ onlyFeatured = false }: { onlyFeatured?: boolean }) {
  const [selectedCursor, setSelectedCursor] = useState<CursorDefinition | null>(null)
  ...
  const filteredCursors = useMemo(() => {
    ...
    return cursors.filter((cursor) => {
      const matchesSearch = cursor.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                            cursor.description.toLowerCase().includes(searchQuery.toLowerCase())
      
      const matchesCategory = selectedCategory === "All" || 
                              cursor.tags.some(tag => tag.toLowerCase().includes(selectedCategory.toLowerCase().replace(" ", "-")))
